$(function() {
		...
		$.getJSON(listUrl, function(data) {
			if (data.success) {
				...
				productList.map(function(item, index) {
					...
					// 拼接每件商品的行信息
					tempHtml += '' + '<div class="row row-product class-row-content">'
							+ '<div class="col-33 class-table-first-col">'
							+ item.productName
							+ '</div>'
							+ '<div class="col-20">'
							+ item.point
	...
	$('#id-button-create').click(function(){
	...
	function changeItemStatus(id, enableStatus) {
		...
		$.confirm('Are you sure to do that?', function() {
			// 上下架相关商品
			$.ajax({
				url : statusUrl,
				type : 'POST',
